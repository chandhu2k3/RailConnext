...
function AnalyticsDashboard(){
 ...
 return <div className="container page analytics-page">
  ...
  <section className="analytics-section"><div className="analytics-section-head"><div><div className="eyebrow">MARKETING SIGNALS</div><h2>Acquisition → activation</h2></div><span className="updated">Updated {new Date(data.last_updated).toLocaleString()}</span></div>
   <div className="kpi-grid marketing-kpis"><MetricCard label="Exposure" value={m.exposure_users||0} sub="Unique sessions shown a tracked feature entry point."/><MetricCard label="CTR" value={`${m.ctr||0}%`} sub="CTA clickers as a share of exposed sessions." accent/><MetricCard label="Search completion" value={`${m.search_completion_rate||0}%`} sub="Users who completed a search out of those who started one."/><MetricCard label="Results reach" value={`${m.results_reach_rate||0}%`} sub="Users who reached results out of completed searches."/><MetricCard label="Unique clickers" value={m.unique_clickers||0} sub="Distinct sessions that clicked a tracked CTA."/><MetricCard label="Search start rate" value={`${m.search_start_rate||0}%`} sub="Search starters as a share of tracked exposed sessions."/><MetricCard label="Post-click drop-off" value={`${m.post_click_dropoff_rate||0}%`} sub={`${m.post_click_dropoff_users||0} CTA clickers did not start a search.`}/></div>
  </section>
  ...
  <div className="analytics-grid">
   <section className="chart-card"><div className="card-heading"><div><span className="eyebrow">WILLINGNESS TO PAY</span><h2>Price sensitivity</h2><p className="card-description"><strong>Shows how many WTP responses selected each price point.</strong> Paid selections are also shown as a share of comparison users when the available data supports that calculation.</p></div></div><div className="price-bars">{wtpRows.map(x=>{const r=Number(x.responses)||0;const pct=wtpTotal?(r/wtpTotal*100):0;return <div key={x.price}><b>₹{x.price}</b><i style={{height:Math.max(8,r*38)+"px"}}></i><span>{r} · {pct.toFixed(1)}%</span></div>})}</div><div className="wtp-summary"><strong>{paidWtp} paid-support selections</strong><span>{wtpTotal?`${paidWtp} of ${wtpTotal} WTP responses · ${paidRate.toFixed(1)}% selected a paid option`:"No WTP responses yet"}{compareUsers?` · ${compareUsers} comparison users`:""}</span></div><p className="chart-note">Responses shown exactly as collected; no inferred preference.</p></section>
  </div>
  <div className="analytics-grid">
